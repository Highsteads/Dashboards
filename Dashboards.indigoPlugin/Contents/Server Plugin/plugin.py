...
# Author:      CliveS & Claude Opus 5 (3.17.0-3.20.0, 3.23.0); Claude Opus 5.5 (3.23.1-3.48.2); Claude Fable 5.1 (3.12.0-3.13.0); Claude Sonnet 5 (2.99.2); Claude Fable 5 (2.79.0); Claude Opus 5 (2.80-2.81, 2.84.0)
# Date:        26-09-2026
# Version:     3.48.2
#
...
class Plugin(CamerasMixin, ConfigMixin, PublishMixin, HealthMixin, ScriptsMixin,
    ...
    def _write_stamp_locked(self, state):
        ...
        # "build" (3.45.8): the running version, so an open page can tell it
        # is running older code than the server now serves and reload itself.
        # A Safari web app left open on the Mac mini ran 3.45.0 for hours
        # after 3.45.7 was installed.
        payload = {"v": 1, "boot": self._boot_ts, "ts": time.time(),
                   "hwm": self._ledger_hwm(), "state": state,
                   "build": str(getattr(self, "pluginVersion", "") or "")}
    ...
    def _setup_checks(self):
        ...
        chk("SigenEnergyManager", sem,
            "present — the Energy and Cost pages are shown" if sem
            else "not installed — the Energy and Cost pages and the hub's energy cards are left out",
            optional=True)
