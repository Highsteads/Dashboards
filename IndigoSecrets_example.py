...
#
#   hideDeviceIds  ids to leave off the page entirely
#   include        ids to force INTO a section the classifier missed
#   sortOrder      the order tiles appear in, per section
#   plugs          mains sockets -> their own section, no bulk on/off
#   tv             TV kit -> its own section, with a bulk on/off
#   fire           an on/off appliance that is neither light nor socket
#   openLoop       devices whose state is a BELIEF, not a reading (a one-way
#                  RF relay). Excluded from any DECISION derived from state,
#                  but still COMMANDED with the rest.
#   mainLight      lights the Lights section's "All On / All Off" must LEAVE
#                  ALONE — the room's main light, typically. It still renders
#                  as its own tile and still switches on its own; it is only
#                  out of the group, so the lamps go off in one press without
#                  also killing the ceiling light. Unlike openLoop it is out
